...
/**
 * Build a GitHub Copilot text-generation closure bound to one instance's
 * settings. Every call spawns a short-lived Copilot ACP session in the
 * requested cwd, pins the session to Plan mode so a commit-message request can
 * never mutate the workspace, and asks for a single structured JSON reply.
 */
    ...
    const runGitHubCopilotJson = <S extends Schema.Top>({
      ...
      Effect.gen(function* () {
        ...
        const promptResult = yield* Effect.gen(function* () {
          yield* runtime.start();
          yield* applyGitHubCopilotSessionConfiguration({
            runtime,
            ...
            mapError: ({ cause, configId }) =>
              new TextGenerationError({
                operation,
                detail: `Failed to set GitHub Copilot ACP config option "${configId}" for text generation.`,
                cause,
              }),
          });
