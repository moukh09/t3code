...
| `githubCopilot` | [`Drivers/GitHubCopilotDriver.ts`][github-copilot] |
...
GitHub Copilot reports Agency's background `task` and `read_agent` calls as generic ACP tool calls.
`GitHubCopilotAgencyTasks.ts` recognizes those bounded input shapes and maps their returned agent ID
and status to canonical `task.started`, `task.updated`, and `task.completed` events. Other dynamic
ACP tools remain ordinary `item.*` events.
...
[github-copilot]: ../../apps/server/src/provider/Drivers/GitHubCopilotDriver.ts
